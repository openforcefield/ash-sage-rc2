...
def main():
    hfes = pd.read_csv("processed_freesolv_validation_stats.csv")
    hfes["Dataset"] = "∆G$_{aq}$"
    sfes = pd.read_csv("processed_mnsol_validation_stats.csv")
    sfes["Dataset"] = "∆G$_{non-aq}$"
    tfes = pd.read_csv("processed_tfes_validation_stats.csv")
    tfes["Dataset"] = "∆G$_{trans}$"

    df = pd.concat([hfes, sfes, tfes])
    STAT_ORDER = ["RMSE", "MSE"]
    ...
    plt.savefig("images/combined-sfes.png", dpi=300)
    print("Saved figure to images/combined-sfes.png")
