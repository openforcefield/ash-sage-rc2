...
def main(
    ...
    df = dataset.to_table(columns=["method", "id"]).to_pandas()
    ...
    dataset = dataset.filter(
        pc.field("id").isin(valid_ids)
    )
    logger.info(f"Filtered to {dataset.count_rows()} with valid ids")
    df = df.to_table().to_pandas()
